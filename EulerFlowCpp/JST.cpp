...
void JST_2ndOrderEulerFlux(flux_state& fluxVector, flux_state& dissipationFlux) {
	...
	double hbar_jphalf, hbar_jmhalf;
	for (size_t iflux = 0; iflux < fluxVector.size(); iflux++) {
		for (size_t i = 1; i < fluxVector[iflux].size() - 1; ++i) {
			hbar_jphalf = (fluxVector[iflux][i + 1] + fluxVector[iflux][i] ) / 2;
			hbar_jmhalf = (fluxVector[iflux][i] + fluxVector[iflux][i - 1]) / 2;
...
void JST_DissipFlux::init(size_t ghost_size) {
	n_nuj = ghost_size;
	nu_j.resize(ghost_size);
	R_jmhalf.resize(ghost_size - 2);
	R_jphalf.resize(ghost_size - 2);
	delta3W_jphalf.resize(ghost_size - 2);
	delta3W_jmhalf.resize(ghost_size - 2);
}

void JST_DissipFlux::operator()(
	flux_state& D_j,				// dissipation flux
	flux_state& W,					// state vector variable
	pde_state& p,					// pressure
	pde_state& u,					// velocity
	...
	for (size_t iflux = 0; iflux < W.size(); ++iflux) {
		...
		for (size_t i = 1; i < W[iflux].size() - 1; ++i) {
			// first derivative of the state vector
			deltaW_jphalf = W[iflux][i + 1] - W[iflux][i];
			deltaW_jmhalf = W[iflux][i] - W[iflux][i - 1];

			// dissipative coefficients, S_(j-1/2) MIGHT NEED FIXING
			S_jphalf = std::max(nu_j[i + 1], nu_j[i]); //std::max(nu_j[i], nu_j[i - 1]);
			S_jmhalf = std::max(nu_j[i], nu_j[i - 1]);

			eps2_jphalf = std::min(alpha[0], alpha[1] * S_jphalf);
			eps2_jmhalf = std::min(alpha[0], alpha[1] * S_jmhalf);
			eps4_jphalf = std::max(0.0, beta[0] - beta[1] * eps2_jphalf);
			eps4_jmhalf = std::max(0.0, beta[0] - beta[1] * eps2_jmhalf);

			// artificial dissipation terms
			d_jphalf = eps2_jphalf * R_jphalf[i - 1] * deltaW_jphalf - eps4_jphalf * R_jphalf[i - 1] * delta3W_jphalf[i - 1];
			d_jmhalf = eps2_jmhalf * R_jmhalf[i - 1] * deltaW_jmhalf - eps4_jmhalf * R_jmhalf[i - 1] * delta3W_jmhalf[i - 1];
			D_j[iflux][i - 1] = d_jphalf - d_jmhalf;
		}
	}
}